...
Отдельные HTML с общими `note.css`/`note.js`; двуязычность через `<span data-lang="ru|en">` + атрибут `data-lang-active` на `<html>`. Формат (v1.1, с 2026-07-09): **связный текст** из 3–6 абзацев с подзаголовками, раскрывающий суть материала; методы описываются развёрнуто (что это, шаги, когда применять); таймкоды видео — инлайн-ссылки `[MM:SS]` прямо в тексте; кадры — `<figure>` рядом с относящимся абзацем. Образцы: страницы за 2026-07-09 в `articles/` и `videos/`. Страницы до 2026-07-09 (включая demo-*) — старый формат «тезисы+таблица таймкодов», не переделываются (архив).
